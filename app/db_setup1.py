...
def run_database_setup(sql_folder="staging"):
    ...
    master_file_path = "run_all_staging.sql"
...
def Generate_master_script(sql_folder="staging"):
    sql_files = [f for f in os.listdir(sql_folder) if f.endswith(".sql")]
    master_file_path = "run_all_staging.sql"

    with open(master_file_path, "w") as f:
        f.write("-- Run all staging tables\n\n")
...
"""
        ...
        for file in sql_files:
            # Force correct Windows-style path
            path = f"{sql_folder}\\{file}"
            f.write(f":r .\\{path}\n")